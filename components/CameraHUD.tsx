...
const CameraHUD: React.FC<CameraHUDProps> = ({ onCapture, analysisResults, isAnalyzing, onReset }) => {
  ...
  const timeoutRef = useRef<any>(null); // Track timeouts for cleanup
      ...
      <div className="absolute bottom-4 right-4 z-30 flex items-center gap-3">
        {(previewUrl || analysisResults) && (
          <button
            ...
            className="px-6 py-2 rounded-sm border border-red-500/30 bg-black/80 hover:bg-red-500/20 transition-all"
          >
            <span className="text-[10px] text-red-500 uppercase tracking-widest font-bold">Clear Evidence</span>
          </button>
        )}
        ...
        {(isActive || (previewUrl && !analysisResults)) && (
          <button
            ...
            className="px-6 py-2 bg-sky-500 hover:bg-sky-400 text-black font-bold text-[10px] tracking-[0.2em] uppercase rounded-sm transition-all shadow-[0_0_15px_rgba(56,189,248,0.4)] disabled:opacity-50"
          >
        ...
        <div className="absolute top-4 left-4 z-30 pointer-events-none">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[8px] font-mono text-white/50 tracking-widest uppercase">
              {isActive ? 'FEED_LIVE_DETERMINISTIC' : analysisResults ? 'POST_ANALYSIS_STATIC' : 'EVIDENCE_LOADED_READY'}
            </span>
          </div>
        </div>
      )}
